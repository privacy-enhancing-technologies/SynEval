...
class DashboardDPEvaluationRunner:
    ...
    def _parse_datasets(self) -> Dict[str, DatasetConfig]:
        ...
        for dataset_key, cfg in datasets_cfg.items():
            ...
            dataset = DatasetConfig(
                ...
                utility_selected_metrics=_ensure_list(utility_cfg.get("selected_metrics"))
                or None,
    ...
    def _expand_generator_group(
        ...
        for epsilon in _ensure_list(epsilons):
            ...
            train_path = (
                ...
                if train_file
                else dataset_cfg.real_train_path
            )
            test_path = (
            ...
            results_dir = self.results_base_dir / group_key / f"eps_{_sanitize_identifier(epsilon)}"
            ...
            experiments.append(
                ExperimentConfig(
                    ...
                    mia_adapter=group_mia_cfg.get("adapter") or self.mia_defaults.get("adapter"),
    ...
    def _build_explicit_experiment(
        ...
        if epsilon is None:
            raise ValueError(f"Explicit experiment for group '{group_key}' missing epsilon.")
        ...
        train_path = self._resolve_optional_path(train_raw) or dataset_cfg.real_train_path
    ...
    def _rename_default_columns(self, df: pd.DataFrame, dataset_cfg: DatasetConfig) -> pd.DataFrame:
        renamed = df.copy()
    ...
    def _load_dataset_column_overrides(self, dataset_cfg: DatasetConfig) -> Dict[str, Dict[str, Any]]:
    ...
    def _generate_metadata(
        ...
        for col in data.columns:
            ...
            if col_meta.get("sdtype") == "categorical" and "categories" not in col_meta:
                unique = data[col].dropna().unique().tolist() if not data[col].empty else []
    ...
    def _call_individual_report_generator(
        ...
        try:
            ...
            logger.error("Failed to generate individual dashboard for %s: %s", exp_config.experiment_id, exc)
    ...
    def _call_in_group_report_generator(
        ...
        if not successful:
            logger.warning("Skipping in-group report for %s (no successful runs).", group_key)
            return
    ...
    def _call_cross_group_report_generator(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning("HTML generator unavailable; skipping cross-group report.")
                return
        try:
            output_dir = self.results_base_dir / "cross_group_comparison"
            generator = DPHtmlGenerator(output_dir=output_dir)
            output_path = generator.generate_cross_group_comparison(all_results=self.group_results)
    ...
    def _run_single_experiment(self, exp_config: ExperimentConfig) -> Dict[str, Any]:
        ...
        if not valid:
            ...
            logger.error("Missing files for %s: %s", exp_config.experiment_id, error_msg)
            return {
                "status": "failed",
                "error": error_msg,
        ...
        try:
            train_data, test_data, synthetic_data = self._load_data(exp_config, dataset_cfg)
        except Exception as exc:
        ...
        if self.privacy_cfg.get("max_rows"):
            ...
            real_data = real_data.sample(n=min(len(real_data), max_rows), random_state=42).reset_index(drop=True)
        ...
        # Fidelity
        fidelity_cfg = (self.evaluations_cfg.get("fidelity") or {}) if self.evaluations_cfg else {}
        if fidelity_cfg.get("enabled", True):
            try:
                selected = fidelity_cfg.get("selected_metrics")
                results["fidelity"] = syneval.evaluate_fidelity(selected_metrics=selected)
            except Exception as exc:
                logger.error("Fidelity evaluation failed for %s: %s", exp_config.experiment_id, exc)
                results["fidelity"] = {"status": "failed", "error": str(exc)}
                failure = True

        # Utility
        utility_cfg = (self.evaluations_cfg.get("utility") or {}) if self.evaluations_cfg else {}
        if utility_cfg.get("enabled", True):
            try:
                selected = dataset_cfg.utility_selected_metrics or utility_cfg.get("selected_metrics")
                ...
                logger.error("Utility evaluation failed for %s: %s", exp_config.experiment_id, exc)
                results["utility"] = {"status": "failed", "error": str(exc)}
                failure = True

        # Privacy
        privacy_cfg = (self.evaluations_cfg.get("privacy") or {}) if self.evaluations_cfg else {}
        if privacy_cfg.get("enabled", True):
            ...
            if self.privacy_cfg.get("skip_anonymeter", True) and "anonymeter" in privacy_metrics:
                privacy_metrics.remove("anonymeter")
            try:
                results["privacy"] = syneval.evaluate_privacy(selected_metrics=privacy_metrics)
            except Exception as exc:
                logger.error("Privacy evaluation failed for %s: %s", exp_config.experiment_id, exc)
                results["privacy"] = {
                    "status": "failed",
                    "error": str(exc),
        ...
        # Diversity
        diversity_cfg = (self.evaluations_cfg.get("diversity") or {}) if self.evaluations_cfg else {}
        if diversity_cfg.get("enabled", True):
            try:
                selected = diversity_cfg.get("selected_metrics")
                results["diversity"] = syneval.evaluate_diversity(selected_metrics=selected)
            except Exception as exc:
                logger.error("Diversity evaluation failed for %s: %s", exp_config.experiment_id, exc)
                results["diversity"] = {"status": "failed", "error": str(exc)}
                failure = True

        # MIA
        mia_cfg = (self.evaluations_cfg.get("mia") or {}) if self.evaluations_cfg else {}
        if mia_cfg.get("enabled", True):
            try:
                from experiments.utils.membership_inference import run_mia_for_experiment  # type: ignore
            except ImportError as exc:
                ...
                try:
                    mia_dataset = exp_config.mia_dataset_name or self.mia_defaults.get("dataset")
                    adapter_name = exp_config.mia_adapter or self.mia_defaults.get("adapter")
                    n_shadow = int(
                    ...
                    logger.error("MIA evaluation failed for %s: %s", exp_config.experiment_id, exc)
    ...
    def _process_experiments(self, experiments: Iterable[ExperimentConfig]) -> None:
        ...
        for group_key, results in self.group_results.items():
            display_name = next(
                (exp.group_display_name for exp in self.experiments if exp.group_key == group_key),
                group_key,
            )
    ...
    def run_sequential(self) -> None:
        logger.info("Starting sequential execution of %d experiments", len(self.experiments))
    ...
    def save_summary(self) -> Path:
        summary = {
            ...
            "successful": sum(1 for r in self.all_results.values() if r.get("status") == "success"),
            "failed": sum(1 for r in self.all_results.values() if r.get("status") == "failed"),
        }
