...
class IndividualDashboardGenerator:
    ...
    def generate(self, results: Dict, experiment_name: Optional[str] = None) -> str:
        ...
        """
        exp_name = experiment_name or results.get('group_display_name', results.get('experiment_id', 'Unknown Experiment'))
...
"""
    ...
    def _summarize_utility_metrics(self, tstr: Dict) -> Dict[str, Any]:
        ...
        summary: Dict[str, Any] = {
            ...
            "training_size": tstr.get("training_size") if isinstance(tstr, dict) else None,
        ...
        def normalize(report: Any) -> Dict[str, float]:
            ...
            for key in ("accuracy", "f1_macro", "precision_macro", "recall_macro", "rmse", "mae", "r2"):
        ...
        is_classification = any(self._is_number(syn_model.get(k)) for k in classification_keys)
        ...
        if is_regression:
            ...
            for name, real_val, syn_val in metric_map:
                if self._is_number(real_val) or self._is_number(syn_val):
                    metrics.append(name)
                    real_values.append(float(real_val) if self._is_number(real_val) else 0.0)
                    syn_values.append(float(syn_val) if self._is_number(syn_val) else 0.0)
            ...
            real_values = []
            syn_values = []
            if self._is_number(real_model.get("accuracy")) or self._is_number(syn_model.get("accuracy")):
                metrics.append("Accuracy")
                real_values.append(
                    float(real_model.get("accuracy")) * 100.0 if self._is_number(real_model.get("accuracy")) else 0.0
                )
                syn_values.append(
                    float(syn_model.get("accuracy")) * 100.0 if self._is_number(syn_model.get("accuracy")) else 0.0
                )
            if self._is_number(real_model.get("f1_macro")) or self._is_number(syn_model.get("f1_macro")):
                metrics.append("F1 Macro")
                real_values.append(
                    float(real_model.get("f1_macro")) * 100.0 if self._is_number(real_model.get("f1_macro")) else 0.0
                )
                syn_values.append(
                    float(syn_model.get("f1_macro")) * 100.0 if self._is_number(syn_model.get("f1_macro")) else 0.0
                )
    ...
    def _generate_overview_cards(self, results: Dict) -> str:
        ...
        # Fidelity Score Card
        fidelity_score = self._safe_get(results, 'fidelity.quality.Overall.score', 0) * 100
        ...
        # Utility Score Card
        utility_summary = self._summarize_utility_metrics(results.get('utility', {}).get('tstr_accuracy', {}))
        utility_score = utility_summary.get('score', 0.0)
        utility_class = self._get_score_class(utility_score)
        utility_subtitle = utility_summary.get('metric_description', 'ML task performance (TSTR)')
        cards_html += f"""
        <div class="col-lg-3 col-md-6">
        ...
        if privacy_summary.get('notes'):
            ...
            privacy_subtitle = note_hint if privacy_score_value is not None and privacy_score_value < 80 else privacy_summary.get('subtitle', 'Structured privacy metrics')
        else:
            privacy_subtitle = privacy_summary.get('subtitle', 'Structured privacy metrics')
        cards_html += f"""
        <div class="col-lg-3 col-md-6">
        ...
        entropy_ratio = self._safe_get(results, 'diversity.tabular_diversity.entropy_metrics.dataset_entropy.entropy_ratio', 0)
    ...
    def _generate_fidelity_section(self, fidelity_data: Dict) -> str:
        ...
        diagnostic_score = fidelity_data.get('diagnostic', {}).get('Overall', {}).get('score', 0) * 100
        html += f"""
        <div class="col-md-6">
        ...
        # Quality Score Gauge
        quality_score = fidelity_data.get('quality', {}).get('Overall', {}).get('score', 0) * 100
        html += f"""
        <div class="col-md-6">
            <div class="chart-container">
    ...
    def _generate_utility_section(self, utility_data: Dict) -> str:
        ...
        html = '<div class="content-section">'
        html += '<h2 class="section-title"><i class="fas fa-cogs"></i>Utility Analysis</h2>'
        html += '<div class="row">'
        html += """
        <div class="col-md-8">
            <div class="chart-container">
        ...
        if summary.get("mode") == "classification":
            ...
            accuracy_block = classification_block("Accuracy", real_model.get("accuracy"), syn_model.get("accuracy"))
            f1_block = classification_block("F1 Macro", real_model.get("f1_macro"), syn_model.get("f1_macro"))
            ...
            <div class="col-md-4">
                <div class="chart-container">
                    <h4 class="chart-title">Key Metrics</h4>
            """
            html += metrics_table or "<p class=\"text-muted small\">No numeric metrics available.</p>"
            html += f"""
                    <hr>
                    <p class="text-muted small">
                        <strong>Task:</strong> {summary.get('task_type', 'Unknown')}<br>
    ...
    def _generate_privacy_section(self, results: Dict) -> str:
        ...
        html = '<div class="content-section">'
        html += '<h2 class="section-title"><i class="fas fa-lock"></i>Privacy Analysis</h2>'
        ...
        if components:
            ...
            for component in components:
                ...
                comp_class = self._get_score_class(comp_score) if comp_score is not None else "warning"
                score_display = f"{comp_score:.1f}%" if comp_score is not None else "N/A"
                value = component.get("value")
                value_display = self._format_float(value, 4) if value is not None else "N/A"
                extras = []
                if component.get("baseline") is not None:
                    extras.append(f"Baseline: {self._format_float(component['baseline'], 4)}")
                if component.get("target") is not None:
                    extras.append(f"Target: {self._format_float(component['target'], 4)}")
                direction_hint = "Higher values indicate stronger privacy." if component.get("direction") == "higher_better" else "Lower values indicate stronger privacy."
        ...
        if structured_metrics:
            ...
            for metric_name, metric in structured_metrics.items():
                ...
                desc = metric.get('desc') or metric.get('method') or 'No description provided.'
        ...
        if isinstance(anonymeter, dict) and anonymeter:
            ...
            for name, block in anonymeter.items():
                if not isinstance(block, dict):
                    continue
                risk = block.get('risk')
                desc = block.get('desc') or block.get('description') or 'No description provided.'
    ...
    def _generate_scripts(self, results: Dict) -> str:
        ...
        diversity_ratio = self._safe_get(results, 'diversity.tabular_diversity.entropy_metrics.dataset_entropy.entropy_ratio', 0)
