...
class DPHtmlGenerator:
    ...
    def __init__(self, output_dir: Union[str, Path] = "reports/dp_evaluation"):
        ...
        logger.info(f"Initialized DP HTML Generator with output directory: {self.output_dir}")
    ...
    def generate_individual_dashboard(
        ...
        logger.info(f"Generating individual dashboard for {experiment_name or results.get('experiment_id', 'unknown')}")
    ...
    def generate_in_group_comparison(
        ...
        if not group_results:
            logger.warning("No successful results provided for in-group comparison '%s'; skipping generation", group_name)
            return None
...
"""
    ...
    def _generate_dashboard_cards(self) -> str:
        ...
        for report in self.generated_reports:
            ...
            relative_path = report_path.relative_to(self.output_dir) if report_path.is_relative_to(self.output_dir) else report_path.name
    ...
    def _generate_summary_stats(self) -> str:
        ...
        counts = {
            'individual': sum(1 for r in self.generated_reports if r['type'] == 'individual'),
            'in_group': sum(1 for r in self.generated_reports if r['type'] == 'in_group'),
            'cross_group': sum(1 for r in self.generated_reports if r['type'] == 'cross_group')
        }
