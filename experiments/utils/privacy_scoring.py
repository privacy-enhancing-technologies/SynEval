# experiments/utils/privacy_scoring.py
...
def compute_privacy_summary(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary["exact_matches"] = privacy_block.get("exact_matches") if isinstance(privacy_block, dict) else None
    ...
    syn_dcr = _to_float(dcr_metric.get("syn_train_5pct")) if isinstance(dcr_metric, dict) else None
    train_dcr = _to_float(dcr_metric.get("train_train_5pct")) if isinstance(dcr_metric, dict) else None
    ...
    if dcr_score is not None:
        ...
        component_details.append({
            ...
            "value": syn_dcr,
            "baseline": train_dcr,
            "target": max(train_dcr or 0.0 + DCR_MIN_BUFFER, (train_dcr or 0.0) * DCR_MULTIPLE, DCR_MIN_BUFFER),
    ...
    syn_nndr = _to_float(nndr_metric.get("syn_train_5pct")) if isinstance(nndr_metric, dict) else None
        ...
        for metric_name, metric_block in anonymeter.items():
            if isinstance(metric_block, dict):
                ...
                component_details.append({
                    ...
                    "value": risk,
                    "direction": "lower_better",
                    "description": metric_block.get("desc") or metric_block.get("description"),
                })
                components.append(score)
                if risk is not None and risk > 0.5:
                    notes.append(f"Anonymeter risk '{metric_name}' is elevated ({risk:.2f}).")
    ...
    if final_score >= 80:
        ...
        summary["message"] = summary.get("message") or "Strong privacy protections detected across structured metrics."
    elif final_score >= 60:
        summary["level"] = "Medium"
        summary["color"] = "#ffc107"
        summary["message"] = summary.get("message") or "Privacy posture is acceptable but warrants monitoring."
    else:
        summary["level"] = "Low"
        summary["color"] = "#dc3545"
        summary["message"] = summary.get("message") or "Structured privacy metrics indicate elevated disclosure risk."
