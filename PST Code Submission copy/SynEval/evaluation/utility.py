...
class UtilityEvaluator:
    ...
    def _get_tstr_cache_fingerprint(self) -> str:
        """Build a fingerprint that represents the current TSTR setup."""
        fingerprint_payload = {
            "original": self.original_fingerprint,
            "synthetic": self.synthetic_fingerprint,
            "input_columns": self.input_columns,
            "output_columns": self.output_columns,
            "task_type": self.task_type,
            "train_split_provided": self.real_train_data is not None,
            "test_split_provided": self.real_test_data is not None,
        }

        # When custom splits are supplied, include their fingerprints to ensure cache validity
        if self.real_train_data is not None:
            fingerprint_payload["real_train_fp"] = compute_dataset_fingerprint(
                self.real_train_data, self.metadata
            )
        if self.real_test_data is not None:
            fingerprint_payload["real_test_fp"] = compute_dataset_fingerprint(
                self.real_test_data, self.metadata
            )

        fingerprint_str = json.dumps(fingerprint_payload, sort_keys=True)
        return hashlib.sha256(fingerprint_str.encode()).hexdigest()
    ...
    def evaluate(self) -> Dict:
        ...
        if "tstr_accuracy" in self.selected_metrics:
            tstr_cache_fingerprint = self._get_tstr_cache_fingerprint()
            cached_tstr = self.cache.load_cache(
                "tstr_accuracy", tstr_cache_fingerprint
            )
            if cached_tstr is not None:
                logger.info("Using cached TSTR utility results.")
                results["tstr_accuracy"] = cached_tstr
            else:
                try:
                    ...
                    if self.task_type in ["classification", "text_classification"]:
                        ...
                        logger.info(
                            ...
                            real_f1
                            if isinstance(real_f1, (int, float))
                            else float("nan"),
                        ...
                        # Cache successful TSTR result
                        self.cache.save_cache(
                            "tstr_accuracy",
                            tstr_cache_fingerprint,
                            results["tstr_accuracy"],
                        )
...
def evaluate_utility_multimodal(
    ...
    This validates that synthetic data preserves predictive relationships between modalities.
    ...
    Returns:
        Dict with structure:
        {
            "text_to_attribute": {
                "target_column": str,
                "f1_score": float  # for classification
                OR "rmse": float   # for regression
            },
            "attribute_to_text": {
                "cluster_prediction_f1": float
            }
        }
    """
    # Split real data 80/20 train/test (using real data for test to measure utility)
    real_train, real_test = train_test_split(
        real_df, test_size=0.2, random_state=42, shuffle=True
    )

    # Get text column name from quantizer
    text_col = quantizer.text_columns[0]
    tabular_cols = quantizer.tabular_columns

    # === Text-to-Attribute (T2A) ===
    # Train classifier on synthetic text embeddings → predict tabular target
    # Evaluate on real test data

    # Get text embeddings for training (synthetic) and test (real)
    ...
    # Check if column is categorical (string/object type) or truly discrete
    is_string_or_categorical = (
        real_df[target_column].dtype == "object"
        or real_df[target_column].dtype.name == "category"
    )

    # For numeric types, check if values are discrete integers with very few unique values
    # AND check if it's actually categorical in nature (e.g., ratings 1-5, not prices)
    ...
    if not is_string_or_categorical and real_df[target_column].dtype in ["int64", "int32"]:
        ...
        # Only treat as classification if < 10 unique values (very discrete)
        if nunique < 10:
            # Check if values are sequential/small integers (0-10 range suggests categorical)
    ...
    if is_classification:
        # Train RandomForestClassifier on synthetic text → target
        ...
        # Calculate F1 score
        t2a_f1 = f1_score(real_test_target, t2a_pred, average="macro")

        text_to_attribute = {
            "target_column": target_column,
            "f1_score": float(t2a_f1),
        }
    else:
        # Train RandomForestRegressor on synthetic text → target
        t2a_model = RandomForestRegressor(n_estimators=100, random_state=42)
        t2a_model.fit(synth_text_embeddings, synth_target)
        t2a_pred = t2a_model.predict(real_test_text_embeddings)

        # Calculate RMSE
        t2a_rmse = np.sqrt(mean_squared_error(real_test_target, t2a_pred))
    ...
    # === Attribute-to-Text (A2T) ===
    # Train classifier on synthetic tabular → predict text clusters
    # Evaluate on real test data

    # Get text clusters for training (synthetic) and test (real)
    ...
    # Get tabular features and encode categorical columns
    ...
    if categorical_cols:
        ...
        if numerical_cols:
            ...
            synth_numerical = synth_tabular_df[numerical_cols].fillna(0).values
            real_test_numerical = real_test_tabular_df[numerical_cols].fillna(0).values

            synth_tabular = np.hstack([synth_numerical, synth_categorical])
            real_test_tabular = np.hstack([real_test_numerical, real_test_categorical])
        else:
    ...
    # Train RandomForestClassifier on synthetic tabular → text clusters
    ...
    # Calculate F1 score
    a2t_f1 = f1_score(real_test_text_clusters, a2t_pred, average="macro")

    attribute_to_text = {
        "cluster_prediction_f1": float(a2t_f1),
    }
