...
try:
    ...
    anonymeter = None  # type: ignore
    InferenceEvaluator = None  # type: ignore
    LinkabilityEvaluator = None  # type: ignore
    SinglingOutEvaluator = None  # type: ignore
    EvaluationResults = None  # type: ignore
...
# Validate that required NLTK datasets are available
...
def ensure_required_nltk_data() -> None:
    missing = []
    for package, resource_path in REQUIRED_NLTK_PACKAGES.items():
        try:
    ...
    if missing:
        ...
        raise LookupError(
            "Missing required NLTK data packages: "
            f"{joined}. Please install them by running "
            "`python -m nltk.downloader punkt punkt_tab stopwords` "
            "or use the setup cell provided in the README/notebook."
        )


ensure_required_nltk_data()
...
def identical_match_share(syn_df, train_df, key_cols=None):
    """
    Calculate the percentage of rows that are identical between synthetic and training data.
    key_cols: Specify primary key columns or all columns
    Returns: IMS score, pass/fail result, detailed description
    """
    if key_cols is None:
    ...
    # IMS between train-test (assumes train_df has test_df available, otherwise use train's own shuffle)
    train_test_ims = 0
    if hasattr(train_df, "test_df") and train_df.test_df is not None:
        ...
        train_test_ims = (
            len(train_set & test_set) / len(train_set) if len(train_set) > 0 else 0
        )
    else:
        # fallback: train's own shuffle
        ...
        train_test_ims = (
            len(train_set & shuffled_set) / len(train_set) if len(train_set) > 0 else 0
        )
...
def prepare_tabular_features(df, cols):
    ...
    datetime_cols = []
    for col in cols:
        if df_copy[col].dtype == "datetime64[ns]" or "datetime" in str(
            df_copy[col].dtype
        ):
            datetime_cols.append(col)
            # Convert datetime to numeric (days since epoch, not seconds)
            df_copy[col] = pd.to_datetime(df_copy[col]).dt.tz_localize(None).astype(
                np.int64
            ) // (10**9 * 86400)
...
def distance_to_closest_records(syn_df, train_df, feature_cols):
    """
    Calculate the distance distribution from each synthetic data point to its nearest neighbor in the training set,
    and compare with the training set's own distribution.
    Returns: 5th percentile, pass/fail result, detailed description
    """
    try:
        combined = pd.concat(
        ...
        # Synthetic-training nearest distance
        ...
        # Training set's own nearest distance (excluding itself)
        dists_train_train, idxs = nn.kneighbors(X_train, n_neighbors=2)
        dists_train_train = dists_train_train[:, 1]  # Skip itself
...
def nearest_neighbor_distance_ratio(syn_df, train_df, feature_cols):
    """
    Calculate the distance ratio distribution from each data point to its nearest and second-nearest neighbors,
    and compare with the training set's own distribution.
    Returns: 5th percentile, pass/fail result, detailed description
    """
    try:
        ...
        # Synthetic-training nearest neighbor distance ratio
        ...
        # Training set's own nearest neighbor distance ratio (excluding itself)
...
class PrivacyEvaluator:
    def __init__(
        ...
        self.cache = SmartCache("artifacts/cache")
        ...
        self.selected_metrics = (
            selected_metrics.copy()
            if selected_metrics
            else self.available_metrics.copy()
        )
        ...
        if device == "auto":
            ...
            if TORCH_AVAILABLE and torch.cuda.is_available():
                ...
                self.logger.warning(
                    f"CUDA requested but unavailable ({warn_reason}). Falling back to CPU."
                )
                self.device = "cpu"
        else:  # cpu
            self.device = "cpu"
        ...
        if "text_privacy" in self.selected_metrics:
            if not TORCH_AVAILABLE:
                self.unavailable_metrics[
                    "text_privacy"
                ] = "requires PyTorch to load Flair NER models"
        ...
        if "text_privacy" in self.selected_metrics and not skip_text_privacy:
            try:
                self.logger.info(
                    "Loading Flair NER model (this may take a few minutes)..."
                )
                ...
                self.unavailable_metrics[
                    "text_privacy"
                ] = f"failed to load Flair model: {e}"
    ...
    def _evaluate_exact_matches(self) -> Dict:
        ...
        """
        # Consider only columns present in both datasets
        common_cols = [
            col
        ...
        if not common_cols:
            self.logger.warning(
                "No common columns found between synthetic and original data for exact match evaluation"
            )
            return {
        ...
        syn_aligned = self.synthetic_data[common_cols].copy()
        orig_aligned = self.original_data[common_cols].copy()

        # Use a sentinel for missing values to ensure consistent grouping
        sentinel = "__SYN_EVAL_MISSING__"
        syn_prepped = syn_aligned.fillna(sentinel)
        orig_prepped = orig_aligned.fillna(sentinel)

        # Compute value counts for each unique row
        syn_counts = syn_prepped.groupby(common_cols, dropna=False).size()
        orig_counts = orig_prepped.groupby(common_cols, dropna=False).size()

        # Align indices and sum the minimum matches for each unique row
        combined = syn_counts.to_frame("synthetic").join(
            orig_counts.to_frame("original"), how="left"
        )
        combined["original"].fillna(0, inplace=True)

        matches = combined.apply(lambda row: min(row["synthetic"], row["original"]), axis=1).sum()
    ...
    def _evaluate_membership_inference(self) -> Dict:
        ...
        if text_columns:
            ...
            non_text_cols = [
                col for col in self.synthetic_data.columns if col not in text_columns
            ]

            if non_text_cols:
                self.logger.info(
                    "Processing non-text columns for membership inference..."
                )
                ...
                synthetic_features = np.hstack(
                    [syn_non_text.values, syn_text_features.toarray()]
                )
                original_features = np.hstack(
                    [orig_non_text.values, orig_text_features.toarray()]
                )
            else:
            ...
            self.logger.info(
                "Processing only non-text columns for membership inference..."
            )
            # Concatenate for consistent one-hot encoding
            combined = pd.concat([self.synthetic_data, self.original_data], axis=0)

            # One-hot encode together
            combined_encoded = pd.get_dummies(combined).fillna(0)

            # Split back to synthetic and original
            synthetic_features = combined_encoded.iloc[
                : len(self.synthetic_data), :
            ].values
            original_features = combined_encoded.iloc[
                len(self.synthetic_data) :, :
            ].values

        self.logger.info(
            f"Feature dimensions - Synthetic: {synthetic_features.shape}, Original: {original_features.shape}"
        )
    ...
    def _prepare_features(
        self, data: pd.DataFrame, vectorizer: Optional[TfidfVectorizer] = None
        ...
        categorical_cols = non_text_data.select_dtypes(
        ...
        if text_columns:
            ...
            if len(non_text_cols) > 0:
                return (
                    np.hstack([non_text_data.values, text_features.toarray()]),
                    vectorizer,
                )
            else:
    ...
    def _evaluate_reidentification_risks(self) -> Dict:
        ...
        try:
            ...
            control_data = orig_data.sample(
                n=min(len(orig_data), 1000), random_state=42
            )
            ...
            singling_out_uni = SinglingOutEvaluator(
                ori=orig_data, syn=syn_data, control=control_data, n_attacks=300
            )
            try:
                ...
                self.logger.info(
                    f"Univariate singling out results - Attack rate: {uni_results.attack_rate}"
                )
                ...
                if uni_results.attack_rate.value <= uni_results.baseline_rate.value:
                    self.logger.warning(
                        "Univariate singling out attack is not better than baseline"
                    )
                ...
                self.logger.warning(
                    f"Univariate singling out evaluation failed: {str(e)}"
                )
                results["singling_out_univariate"] = {
                    "error": str(e),
                    "risk_level": "unknown",
            ...
            try:
                ...
                self.logger.info(
                    f"Multivariate singling out results - Attack rate: {multi_results.attack_rate}"
                )
                ...
                if multi_results.attack_rate.value <= multi_results.baseline_rate.value:
                    self.logger.warning(
                        "Multivariate singling out attack is not better than baseline"
                    )
                ...
                self.logger.warning(
                    f"Multivariate singling out evaluation failed: {str(e)}"
                )
                results["singling_out_multivariate"] = {
                    "error": str(e),
                    "risk_level": "unknown",
            ...
            try:
                ...
                self.logger.info(
                    f"Linkability results - Attack rate: {link_results.attack_rate}"
                )
                ...
                if link_results.attack_rate.value <= link_results.baseline_rate.value:
                    self.logger.warning(
                        "Linkability attack is not better than baseline"
                    )
                    results["linkability"] = {
                ...
                results["linkability"] = {"error": str(e), "risk_level": "unknown"}

            # 4. Inference Attack
            self.logger.info("Starting inference attack...")
            inference_results = []

            secrets = list(orig_data.columns)

            for secret in tqdm(
                secrets,
                desc="Inference attacks",
                unit="column",
                dynamic_ncols=True,
                leave=False,
            ):
                self.logger.info(
                    f"Evaluating inference risk for secret column: {secret}"
                )
                ...
                n_attacks = min(
                    100, len(orig_data) // 2
                )  # Use at most 100 attacks or half the data size
                ...
                try:
                    ...
                    self.logger.info(
                        f"Inference results for {secret} - Attack rate: {inf_results.attack_rate}"
                    )
                    ...
                    if inf_results.attack_rate.value <= inf_results.baseline_rate.value:
                        self.logger.warning(
                            f"Inference attack for {secret} is not better than baseline"
                        )
                    ...
                    self.logger.warning(
                        f"Inference evaluation failed for {secret}: {str(e)}"
                    )
                    inference_results.append(
                        {
                            "secret_column": secret,
            ...
            overall_risk = max(
                ...
                max(
                    [r.get("risk", 0) for r in results.get("inference", [])], default=0
                ),
            )
    ...
    def _generate_privacy_visualizations(self, results: Dict):
        ...
        try:
            visualization_dir = Path("artifacts/html/privacy")
            ...
            if "inference" in results and results["inference"]:
                ...
                columns = [
                    r["secret_column"] for r in results["inference"] if "risk" in r
                ]

                if risks and columns:  # Only plot if we have valid data
                    plt.bar(
                        x=columns, height=risks, alpha=0.5, ecolor="black", capsize=10
                    )
                    ...
                    plt.tight_layout()
                    plt.savefig(visualization_dir / "inference_risk.png")
                plt.close()
            ...
            plt.tight_layout()
            plt.savefig(visualization_dir / "attack_success_rates.png")
            plt.close()

            # 3. Risk Scores Heatmap
            risk_scores = {
                "Singling Out (Uni)": results.get("singling_out_univariate", {}).get(
                    "risk", 0
                ),
                "Singling Out (Multi)": results.get(
            ...
            plt.tight_layout()
            plt.savefig(visualization_dir / "risk_scores_heatmap.png")
            plt.close()
    ...
    def _process_entities_batch(self, texts: List[str]) -> List[Tuple]:
        ...
        if self.ner_tagger is None:
            self.logger.warning(
                "Skipping entity processing because the Flair NER model is unavailable."
            )
            return []
            ...
            for i in range(0, len(texts), batch_size):
                ...
                for sentence in sentences:
                    ...
                    for entity in sentence.get_spans("ner"):
                        ...
                        if entity.text.isdigit() or not any(
                            c.isalnum() for c in entity.text
                        ):
                            continue

                        # Skip if entity contains URLs or email addresses
                        if (
                            "http" in entity.text.lower()
                            or "@" in entity.text
                            or "." in entity.text.split()[-1]
                        ):
                            continue
                    ...
                    if self.nlp is not None:
                        ...
                        num_tokens = len(
                            [word for word in sentence.text.split() if word.strip()]
                        )
            ...
            for i in range(0, len(texts), batch_size):
                ...
                for doc in docs:
                    ...
                    if doc is None:
                        ...
                        num_tokens = len(
                            [word for word in batch_texts[0].split() if word.strip()]
                        )
                        results.append(
                            (valid_nominals, len(valid_nominals), num_tokens)
                        )
                        continue
                    ...
                    for token in doc:
                        ...
                        if token.text.isdigit() or not any(
                            c.isalnum() for c in token.text
                        ):
                            continue
                        ...
                        if (
                            ...
                            or token.text.lower() in skip_words
                        ):
                            continue
                        ...
                        if token.pos_ == "NOUN" and token.lemma_ in {
                            "be",
                            "have",
                            "do",
                            "get",
                        ...
                        if token.pos_ == "PROPN" and (
                            len(token.text) <= 1 or token.text.isdigit()
                        ):
                            continue
                        ...
                        # 3. Check if it's a noun representing a role or relationship
                            ...
                            token.text.lower() in pattern_set
    ...
    def _analyze_named_entities(self) -> Dict:
        ...
        cached_orig_data = self.cache.load_cache(
            "named_entities_original", self.original_fingerprint, text_column
        )
        if cached_orig_data is None:
            ...
            self.cache.save_cache(
                ...
                {"entities_by_type": group_entities(orig_entities)},
                text_column,
            )
        else:
        ...
        self.cache.save_cache(
            "named_entities_synthetic",
            self.synthetic_fingerprint,
            {"entities_by_type": group_entities(syn_entities)},
            text_column,
        )
        ...
        syn_avg_entity_density = (
            syn_total_entities / syn_total_tokens if syn_total_tokens > 0 else 0
        )

        orig_total_entities = len(orig_entities)
        orig_total_tokens = (
            sum(num_tokens for _, _, num_tokens in orig_results)
            if "orig_results" in locals()
            else 0
        )
        orig_avg_entity_density = (
            orig_total_entities / orig_total_tokens if orig_total_tokens > 0 else 0
        )

        # Calculate entity overlap
        common_entities = syn_entities.intersection(orig_entities)
        overlap_percentage = (
            (len(common_entities) / len(orig_entities) * 100) if orig_entities else 0
        )
    ...
    def _analyze_nominal_mentions(self) -> Dict:
        ...
        cached_orig_nominals = self.cache.load_cache(
            "nominal_mentions_original", self.original_fingerprint, text_column
        )
        if cached_orig_nominals is None:
            self.logger.info(
                "Processing original data nominal mentions (not in cache)..."
            )
            orig_texts = self.original_data[text_column].astype(str).tolist()
            ...
            self.cache.save_cache(
                "nominal_mentions_original",
                self.original_fingerprint,
                {"nominals": list(orig_nominals)},
                text_column,
            )
        else:
        ...
        self.cache.save_cache(
            "nominal_mentions_synthetic",
            self.synthetic_fingerprint,
            {"nominals": list(syn_nominals)},
            text_column,
        )
        ...
        syn_total_tokens = sum(num_tokens for _, _, num_tokens in syn_results)
        syn_avg_nominal_density = (
            syn_total_nominals / syn_total_tokens if syn_total_tokens > 0 else 0
        )
        ...
        orig_avg_nominal_density = (
            orig_total_nominals / orig_total_tokens if orig_total_tokens > 0 else 0
        )

        # Calculate nominal mention overlap
        common_nominals = syn_nominals.intersection(orig_nominals)
        overlap_percentage = (
            (len(common_nominals) / len(orig_nominals) * 100) if orig_nominals else 0
        )
    ...
    def _analyze_stylistic_outliers(self) -> Dict:
        ...
        cached_orig_outliers = self.cache.load_cache(
            "stylistic_outliers_original", self.original_fingerprint, text_column
        )
        if cached_orig_outliers is None:
            self.logger.info(
                "Processing original data stylistic outliers (not in cache)..."
            )
            orig_texts = self.original_data[text_column].astype(str).tolist()
            ...
            orig_outliers = np.array(
                [
                    text in outlier_texts
                    for text in self.original_data[text_column].astype(str).tolist()
                ]
            )
        ...
        orig_outlier_count = (
            len(cached_orig_outliers["outlier_texts"]) if cached_orig_outliers else 0
        )
        ...
        return {
            "synthetic": {
                ...
                "risk_level": (
                    "high" if len(outlier_texts) / len(syn_texts) > 0.1 else "low"
                ),
            },
            "original": {
                "num_outliers": orig_outlier_count,
                "outlier_percentage": float(
                    orig_outlier_count / len(self.original_data) * 100
                ),
                "risk_level": (
                    "high"
                    if orig_outlier_count / len(self.original_data) > 0.1
                    else "low"
                ),
            },
            "comparison": {
                "outlier_ratio": (
                    float(len(outlier_texts) / orig_outlier_count)
                    if orig_outlier_count > 0
                    else float("inf")
                ),
                "risk_level": (
                    "high"
                    if abs(
                        len(outlier_texts) / len(syn_texts)
                        - orig_outlier_count / len(self.original_data)
                    )
                    > 0.1
                    else "low"
                ),
    ...
    def _get_text_column(self) -> Optional[str]:
        ...
        if "text_columns" in self.metadata:
            ...
            if text_col in self.synthetic_data.columns:
                ...
                self.logger.warning(
                    f"Text column '{text_col}' from metadata not found in data"
                )
                return None
        return None
    ...
    def evaluate_structured_privacy_metrics(self, feature_cols=None, key_cols=None):
        """
        Evaluate privacy metrics for structured data, including IMS, DCR, NNDR.
        feature_cols: Feature columns used for distance calculation
        key_cols: Primary key columns used for exact matching
        Returns: dict containing detailed scores and pass/fail results for each metric
        """
        self.logger.info(
            f"Evaluating structured privacy metrics with feature_cols: {feature_cols}, key_cols: {key_cols}"
        )
        ...
        all_tabular_cols = [
            ...
            and col in self.original_data.columns
        ]

        # For IMS, we should exclude PII columns to avoid exact matches
        non_pii_tabular_cols = [
            col
            for col in all_tabular_cols
        ...
        if feature_cols:
            ...
            feature_cols = (
                non_pii_tabular_cols if non_pii_tabular_cols else all_tabular_cols
            )
        ...
        if key_cols:
            self.logger.info(
                f"Sample synthetic row for IMS: {self.synthetic_data[key_cols].iloc[0].to_dict()}"
            )
            self.logger.info(
                f"Sample original row for IMS: {self.original_data[key_cols].iloc[0].to_dict()}"
            )

        # Ensure we have at least some columns to work with
        if not feature_cols:
            self.logger.warning(
                "No feature columns available for structured privacy metrics"
            )
            return {
        ...
        try:
            ims = identical_match_share(
                self.synthetic_data, self.original_data, key_cols=key_cols
            )
        ...
        try:
            dcr = distance_to_closest_records(
                self.synthetic_data, self.original_data, feature_cols=feature_cols
            )
        ...
        try:
            nndr = nearest_neighbor_distance_ratio(
                self.synthetic_data, self.original_data, feature_cols=feature_cols
            )
...
def _get_interpretation(pct_at_risk: float) -> str:
    ...
    Args:
        pct_at_risk: Percentage of synthetic records below risk threshold
...
def evaluate_privacy_multimodal(
    ...
    This metric combines text embeddings and tabular features into a weighted joint space,
    ...
    Args:
        ...
        alpha: Weight balancing text vs tabular (default: 0.5)
               - alpha=1.0: only text embeddings
               - alpha=0.0: only tabular features
               - alpha=0.5: equal weight
        risk_threshold: Distance threshold for memorization risk (default: 0.1)

    Returns:
        Dictionary containing:
            - semantic_dcr_mean: Mean DCR across all synthetic samples
            - semantic_dcr_min: Minimum DCR (closest synthetic point to any real point)
            - semantic_dcr_median: Median DCR
            - pct_below_threshold: Percentage of synthetic points below risk threshold
    ...
    categorical_cols = real_tabular.select_dtypes(
        include=["object", "category"]
    ).columns.tolist()
    ...
    # Normalize tabular features with StandardScaler
    scaler = StandardScaler()
    real_tabular_scaled = scaler.fit_transform(real_tabular)
    synth_tabular_scaled = scaler.transform(synth_tabular)

    # Construct weighted joint space: [alpha * text_emb, (1-alpha) * tabular]
    real_joint = np.hstack(
        [alpha * real_text_emb, (1 - alpha) * real_tabular_scaled]
    )
    synth_joint = np.hstack(
        [alpha * synth_text_emb, (1 - alpha) * synth_tabular_scaled]
    )

    # L2-normalize the joint space vectors
    real_joint_normalized = normalize(real_joint, norm="l2")
    synth_joint_normalized = normalize(synth_joint, norm="l2")

    # Compute DCR: for each synthetic point, minimum Euclidean distance to any real point
    ...
    # Compute percentage below threshold (memorization risk)
    ...
    return {
        "semantic_dcr_mean": semantic_dcr_mean,
        "semantic_dcr_min": semantic_dcr_min,
        "semantic_dcr_median": semantic_dcr_median,
