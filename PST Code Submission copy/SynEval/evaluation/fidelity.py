...
try:
    from sdv.evaluation.single_table import evaluate_quality  # type: ignore
...
class FidelityEvaluator:
    def __init__(
        ...
        """
        # SDV is optional. When unavailable (e.g., unsupported Python version),
        # we still provide non-SDV fidelity signals (text + numerical statistics).
        if not SDV_AVAILABLE:
            logger.warning(
                "Optional dependency 'sdv' unavailable; skipping SDV diagnostic/quality fidelity. "
    ...
    def evaluate(self) -> Dict:
        ...
        if "diagnostic" in self.selected_metrics and sdv_metadata:
            try:
                ...
                results["diagnostic"] = {
                    "Data Validity": round(
                        diagnostic_report.get_details("Data Validity")["Score"].mean(),
                        4,
                    ),
                    "Data Structure": round(
                        diagnostic_report.get_details("Data Structure")["Score"].mean(),
                        4,
                    ),
                    "Overall": {"score": round(diagnostic_report.get_score(), 4)},
                }

                logger.info(
                    f"Diagnostic evaluation completed. Overall score: {results['diagnostic']['Overall']['score']}"
                )
            except Exception as e:
                ...
                results["diagnostic"] = {
                    ...
                    "Overall": {
                        "score": (
                            round(diagnostic_report.get_score(), 4)
                            if "diagnostic_report" in locals()
                            else None
                        )
                    },
                }
        ...
        if "quality" in self.selected_metrics and sdv_metadata:
            try:
                ...
                results["quality"] = {
                    "Column Shapes": round(
                        quality_report.get_details("Column Shapes")["Score"].mean(), 4
                    ),
                    "Column Pair Trends": round(
                        quality_report.get_details("Column Pair Trends")[
                            "Score"
                        ].mean(),
                        4,
                    ),
                    "Overall": {"score": round(quality_report.get_score(), 4)},
                }

                logger.info(
                    f"Quality evaluation completed. Overall score: {results['quality']['Overall']['score']}"
                )
            except Exception as e:
                ...
                results["quality"] = {
                    ...
                    "Overall": {
                        "score": (
                            round(quality_report.get_score(), 4)
    ...
    def _evaluate_text(self, columns: list) -> Dict:
        ...
        for col in text_cols:
            ...
            try:
                ...
                original_tfidf = vectorizer.fit_transform(
                    self.original_data[col].fillna("")
                )
                synthetic_tfidf = vectorizer.transform(
                ...
                if self.device == "cuda":
                    ...
                    original_scores = torch.tensor(
                        original_tfidf.mean(axis=0).A1, device="cuda"
                    )
                    synthetic_scores = torch.tensor(
                        synthetic_tfidf.mean(axis=0).A1, device="cuda"
                    )
                ...
                original_keyword_scores = sorted(
                    zip(original_keywords, original_scores),
                    key=lambda x: x[1],
                    reverse=True,
                )
                synthetic_keyword_scores = sorted(
                    zip(original_keywords, synthetic_scores),
                    key=lambda x: x[1],
                    reverse=True,
                )
            ...
            try:
                ...
                def get_sentiment_percentages(sentiments):
                    ...
                    neutral = (
                        sum(1 for s in sentiments if -0.1 <= s <= 0.1) / total * 100
                    )
                ...
                col_results["sentiment_analysis"] = {
                    ...
                    "original_sentiment_distribution": get_sentiment_percentages(
                        original_sentiments
                    ),
                    "synthetic_sentiment_distribution": get_sentiment_percentages(
                        synthetic_sentiments
                    ),
                }
            except Exception as e:
    ...
    def _evaluate_numerical_statistics(self) -> Dict:
        ...
        for col in numerical_cols:
            try:
                ...
                relative_diffs = {
                    "mean_diff": calc_relative_diff(
                        orig_stats["mean"], syn_stats["mean"]
                    ),
                    "median_diff": calc_relative_diff(
                        orig_stats["median"], syn_stats["median"]
                    ),
                    "std_diff": calc_relative_diff(orig_stats["std"], syn_stats["std"]),
                    "skewness_diff": abs(
                        orig_stats["skewness"] - syn_stats["skewness"]
                    ),
                    "kurtosis_diff": abs(
                        orig_stats["kurtosis"] - syn_stats["kurtosis"]
                    ),
                }
                ...
                if bins > 1:
                    if self.device == "cuda":
                        ...
                        orig_tensor = torch.tensor(
                            orig_data.values, device="cuda", dtype=torch.float32
                        )
                        syn_tensor = torch.tensor(
                            syn_data.values, device="cuda", dtype=torch.float32
                        )
                        ...
                        kl_div = torch.sum(
                            orig_hist
                            * torch.log((orig_hist + 1e-10) / (syn_hist + 1e-10))
                        )
                        ...
                        kl_div = np.sum(
                            orig_hist * np.log((orig_hist + 1e-10) / (syn_hist + 1e-10))
                        )
                ...
                fidelity_components = [
                    ...
                    1
                    - min(
                        relative_diffs["std_diff"], 1
                    ),  # Standard deviation preservation
                    1
                    - min(
                        relative_diffs["skewness_diff"] / 2, 1
                ...
                results[col] = {
                    ...
                    "fidelity_interpretation": self._interpret_fidelity_score(
                        overall_fidelity
                    ),
                }
                ...
                logger.error(
                    f"Error evaluating numerical statistics for column {col}: {str(e)}"
                )
    ...
    def _interpret_fidelity_score(self, score: float) -> str:
        ...
        if score >= 0.9:
            ...
            return (
                "Good fidelity - synthetic data preserves most statistical properties"
            )
        elif score >= 0.7:
            return (
                "Fair fidelity - synthetic data preserves basic statistical properties"
            )
        elif score >= 0.6:
...
def compute_jsd(P: np.ndarray, Q: np.ndarray) -> float:
    ...
    # Flatten to 1D for jensenshannon function
    P_flat = P.flatten()
    Q_flat = Q.flatten()
    ...
    # Compute JSD using scipy (returns value in [0, 1])
    # Note: scipy's jensenshannon returns sqrt(JSD), so we need to square it
    jsd_value = jensenshannon(P_norm, Q_norm, base=2)

    # Square the result to get actual JSD
    jsd_squared = jsd_value ** 2

    return float(jsd_squared)
...
def evaluate_fidelity_multimodal(
    ...
    This function computes Jensen-Shannon Divergence over the joint distribution
    of quantized multimodal data. It detects cross-modal hallucinations where
    marginal distributions look good but the joint distribution is broken
    (e.g., Tilted Data scenario).
    ...
    Returns:
        Dict containing:
            - joint_spectral_divergence: JSD over joint distribution [0, 1]
            - marginal_text_divergence: JSD over text marginal [0, 1]
            - marginal_tabular_divergence: JSD over tabular marginal [0, 1]
            - interpretation: Human-readable quality assessment

    Example:
        >>> from evaluation.quantization.semantic_quantizer import SemanticQuantizer
        >>> quantizer = SemanticQuantizer(text_columns=["text"], tabular_columns=["value"])
        >>> quantizer.fit(real_df)
        >>> result = evaluate_fidelity_multimodal(real_df, synth_df, quantizer)
        >>> print(result["joint_spectral_divergence"])
        0.31
    """
    # Transform both datasets to quantized space
    real_quantized = quantizer.transform(real_df)
    synth_quantized = quantizer.transform(synth_df)

    # Get joint distributions (2D contingency tables)
    real_joint = quantizer.get_joint_distribution(real_quantized)
    synth_joint = quantizer.get_joint_distribution(synth_quantized)

    # Compute JSD on joint distribution
    joint_jsd = compute_jsd(real_joint, synth_joint)

    # Compute marginal distributions for comparison
    real_marginal_text = real_joint.sum(axis=1)  # Sum over tabular dimension
    synth_marginal_text = synth_joint.sum(axis=1)

    real_marginal_tabular = real_joint.sum(axis=0)  # Sum over text dimension
    synth_marginal_tabular = synth_joint.sum(axis=0)

    # Compute marginal JSD values
