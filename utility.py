    ...
    def evaluate(self) -> Dict:
        ...
        if "tstr_accuracy" in self.selected_metrics:
            try:
                ...
                logger.info(
                    f"Using all synthetic data ({syn_size} samples) for training"
                )
                ...
                if len(self.original_data) > syn_size:
                    ...
                    test_data = (
                        pd.DataFrame()
                    )  # Empty test set if real data is smaller than synthetic
                    logger.warning(
                        "Real data is smaller than synthetic data. No test set available."
                    )
                ...
                if DEVICE == "cuda" and not hasattr(
