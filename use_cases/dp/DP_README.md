...
**Minimal configuration (`use_cases/dp/configs/simple_eval.yaml`):**
...
```bash
python use_cases/dp/run_experiment.py \
    --config use_cases/dp/configs/simple_eval.yaml \
...
**Configuration (`use_cases/dp/configs/single_epsilon.yaml`):**
...
```bash
python use_cases/dp/run_experiment.py \
    --config use_cases/dp/configs/single_epsilon.yaml \
    --group DP_EPSILON_10 \
    --epsilon 10
```
...
**Configuration (`use_cases/dp/configs/compare_methods.yaml`):**
...
```bash
python use_cases/dp/run_dp_evaluation.py --config use_cases/dp/configs/compare_methods.yaml
```
...
```bash
python use_cases/dp/run_experiment.py \
    --config use_cases/dp/configs/compare_methods.yaml \
    --group GAN_METHOD \
    --epsilon none
```
...
experiment_groups:
  - key: OUTPUT_GRID_ROW_DP_EXTREMA # Unique identifier for this group
    ...
    generator: # Pattern-based path generation
      type: pattern
      base_dir: use_cases/dp/data/OUTPUT_GRID_ROW_DP_EXTREMA
...
experiment_groups:
  - key: MY_EXPERIMENT
    ...
    generator:
      type: pattern
      base_dir: use_cases/dp/data/MY_EXPERIMENT
...
- `use_cases/dp/data/MY_EXPERIMENT/dp_eps1/data_subfolder/synthetic.csv`
- `use_cases/dp/data/MY_EXPERIMENT/dp_eps10/data_subfolder/synthetic.csv`
...
```bash
python use_cases/dp/run_experiment.py \
    --config use_cases/dp/configs/your_config.yaml \
...
```bash
python use_cases/dp/run_dp_evaluation.py --config use_cases/dp/configs/cross_group_dp.yaml
```
...
```bash
# Run in parallel
python use_cases/dp/run_dp_evaluation.py \
    --config use_cases/dp/configs/cross_group_dp.yaml \
    --parallel \
    --workers 8
...
ls use_cases/dp/data/OUTPUT_GRID_ROW_DP_EXTREMA/dp_eps10/stocks/ddpm_fake_stocks.csv
...
- Check YAML syntax is valid:
  ```bash
  python -c "import yaml; yaml.safe_load(open('use_cases/dp/configs/your_config.yaml'))"
  ```
...
1. **Validate file paths:**
   ...
   config = yaml.safe_load(open('use_cases/dp/configs/your_config.yaml'))
...
```bash
python use_cases/dp/run_experiment.py --config config.yaml --group MY_DATA --epsilon none
```
...
```bash
python use_cases/dp/run_experiment.py --config config.yaml --group DP_EPS10 --epsilon 10
```
...
```bash
# Run all
python use_cases/dp/run_dp_evaluation.py --config config.yaml

# Or run one
python use_cases/dp/run_experiment.py --config config.yaml --group DP_SWEEP --epsilon 10
```
...
# All experiments
python use_cases/dp/run_dp_evaluation.py --config use_cases/dp/configs/your_config.yaml
```

Your HTML dashboards will appear in `reports/dp_evaluation/`! 🎉

$ open reports/dp*evaluation/SINGLE_EVAL/eps_none/individual*\*.html
