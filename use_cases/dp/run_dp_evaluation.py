...
class DashboardDPEvaluationRunner:
    ...
    def __init__(self, config_path: str = "use_cases/dp/configs/cross_group_dp.yaml") -> None:
    ...
    def _call_individual_report_generator(
        ...
        if not self.report_cfg.get("individual_reports", True):
            return
        try:
            from use_cases.dp.utils.dp_html_generator import \
    ...
    def _run_single_experiment(self, exp_config: ExperimentConfig) -> Dict[str, Any]:
        ...
        if mia_cfg.get("enabled", True):
            try:
                from use_cases.dp.utils.membership_inference import \
...
def main(default_config: str = "use_cases/dp/configs/cross_group_dp.yaml") -> int:
