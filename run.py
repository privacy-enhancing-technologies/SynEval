...
def parse_args():
    ...
    parser.add_argument('--correlation-text-col', default='review', help='Text column for correlation analysis')
    parser.add_argument('--correlation-rating-col', default='rating', help='Rating column for sentiment correlation')
    parser.add_argument('--correlation-category-col', default='category', help='Category column for keyword correlation')
    parser.add_argument('--correlation-numeric-col', default='price', help='Numeric column for length correlation')
    parser.add_argument('--correlation-pii-col', default='user_id', help='PII column for leakage correlation')
    parser.add_argument('--correlation-top-n', type=int, default=50, help='Top N keywords for keyword-category correlation')
        ...
        if dimension == 'fidelity':
            ...
            if 'tstr_accuracy' in dimension_results:
                ...
                if 'real_data_model' in tstr_results and 'synthetic_data_model' in tstr_results:
                    ...
                    if isinstance(real_acc, (int, float)) and isinstance(syn_acc, (int, float)) and real_acc != 0:
                        ratio = syn_acc / real_acc
                        print(f"Performance Ratio: {ratio:.3f}")
                    else:
                        print("Performance Ratio: N/A (cannot calculate)")
                else:
                    print("TSTR accuracy results found but missing model performance data")
            # Fallback: check for direct model results (legacy format)
            elif 'real_data_model' in dimension_results and 'synthetic_data_model' in dimension_results:
                        ...
                        if 'semantic_diversity' in col_results:
                            ...
                            if 'distinct_ratio' in sem_div:
                                ...
                                if isinstance(distinct_ratio, (int, float)):
                                    print(f"Text Semantic Diversity: {distinct_ratio:.3f}")
                                else:
                        ...
                        if 'sentiment_diversity' in col_results:
                            ...
                            if 'sentiment_alignment_score' in sent_div:
                                ...
                                if isinstance(alignment_score, (int, float)):
                                    print(f"Sentiment Alignment Score: {alignment_score:.3f}")
                                else:
                                    print(f"Sentiment Alignment Score: {alignment_score}")
            ...
            if 'membership_inference' in dimension_results:
                ...
                if isinstance(syn_confidence, (int, float)) and isinstance(orig_confidence, (int, float)):
