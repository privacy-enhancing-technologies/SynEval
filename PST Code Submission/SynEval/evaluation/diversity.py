...
# Validate required NLTK datasets are available
...
def compute_joint_entropy(joint_prob: np.ndarray) -> float:
    ...
    """
    # Flatten to 1D for easier iteration
...
def evaluate_diversity_multimodal(
    ...
    Args:
        ...
        quantizer: MultimodalQuantizer instance for quantizing data

    Returns:
        Dict with structure:
        {
            "multimodal_metrics": {
                "joint_shannon_entropy": float,  # H(X,Y) for synthetic
                "real_joint_entropy": float,      # H(X,Y) for real
                "diversity_ratio": float,         # synth/real
                "interpretation": str             # Assessment of mode collapse
            },
            "unimodal_metrics": {
                "text_marginal_entropy": float,    # H(X)
                "tabular_marginal_entropy": float  # H(Y)
            }
        }
    """
    try:
        ...
        # Compute marginal entropies for comparison
        # Text marginal: sum over tabular dimension
        real_text_marginal = real_joint_dist.sum(axis=1)
        synth_text_marginal = synth_joint_dist.sum(axis=1)

        # Tabular marginal: sum over text dimension
        real_tabular_marginal = real_joint_dist.sum(axis=0)
        synth_tabular_marginal = synth_joint_dist.sum(axis=0)

        # Compute marginal entropies using the same formula
        text_marginal_entropy = compute_joint_entropy(
            synth_text_marginal.reshape(-1, 1)
        )
        tabular_marginal_entropy = compute_joint_entropy(
            synth_tabular_marginal.reshape(-1, 1)
        )
...
class DiversityEvaluator:
    def __init__(
        ...
        self.available_metrics = ["tabular_diversity", "text_diversity"]
    ...
    def _evaluate_numerical_diversity(self, col: str) -> Dict:
        ...
        try:
            ...
            stat_diffs = {
                "mean_diff": abs(real_stats["mean"] - syn_stats["mean"])
                / (abs(real_stats["mean"]) + 1e-10),
                "std_diff": abs(real_stats["std"] - syn_stats["std"])
                / (abs(real_stats["std"]) + 1e-10),
                "skew_diff": abs(real_stats["skew"] - syn_stats["skew"]),
                "kurtosis_diff": abs(real_stats["kurtosis"] - syn_stats["kurtosis"]),
            }

            # Value range coverage
            overlap = min(real_stats["max"], syn_stats["max"]) - max(
                real_stats["min"], syn_stats["min"]
            )
            ...
            quartile_coverage = {
                "q1": abs(real_quartiles[0.25] - syn_quartiles[0.25])
                / (abs(real_quartiles[0.25]) + 1e-10),
                "q2": abs(real_quartiles[0.5] - syn_quartiles[0.5])
                / (abs(real_quartiles[0.5]) + 1e-10),
                "q3": abs(real_quartiles[0.75] - syn_quartiles[0.75])
                / (abs(real_quartiles[0.75]) + 1e-10),
            }
            ...
            kl_div = np.sum(
                real_hist * np.log((real_hist + 1e-10) / (syn_hist + 1e-10))
            )
            ...
            return {
                ...
                "quartile_coverage": {
                    k: (1 - v) * 100 for k, v in quartile_coverage.items()
                },
                "distribution_similarity": {
                    "kl_divergence": kl_div,
                    "similarity_score": 100
                    * np.exp(-kl_div),  # Convert to similarity score
                },
            ...
            logger.error(
                f"Error evaluating numerical diversity for column {col}: {str(e)}"
            )
            return {}
    ...
    def _evaluate_categorical_diversity(self, col: str) -> Dict:
        ...
        try:
            ...
            category_coverage = (
                len(common_categories) / len(real_counts) if len(real_counts) > 0 else 0
            )
            ...
            rare_coverage = (
                len(rare_categories & set(syn_counts.index)) / len(rare_categories)
                if rare_categories
                else 0
            )
            ...
            logger.error(
                f"Error evaluating categorical diversity for column {col}: {str(e)}"
            )
            return {}
        ...
        try:
            ...
            for col in self.structured_columns:
                try:
                    ...
                    if col_type == "numerical":
                        real_entropy = self._calculate_numerical_entropy(
                            self.original_data[col]
                        )
                        syn_entropy = self._calculate_numerical_entropy(
                            self.synthetic_data[col]
                        )
                    else:  # categorical
                        real_entropy = self._calculate_categorical_entropy(
                            self.original_data[col]
                        )
                        syn_entropy = self._calculate_categorical_entropy(
                            self.synthetic_data[col]
                        )
                    ...
                    logger.error(
                        f"Error calculating entropy for column {col}: {str(e)}"
                    )
                    continue
            ...
            if num_columns > 0:
                results["dataset_entropy"] = {
                    ...
                    "entropy_ratio": (total_syn_entropy / num_columns)
                    / ((total_real_entropy / num_columns) + 1e-10),
                }
    ...
    def evaluate_tabular_diversity(self) -> Dict:
            ...
            try:
                ...
                if col_type == "numerical":
                    ...
                    results["numerical_metrics"][
                        col
                    ] = self._evaluate_numerical_diversity(col)
                    ...
                    coverage = len(real_categories.intersection(syn_categories)) / len(
                        real_categories
                    )
                    results["coverage"][col] = coverage * 100

                    # Enhanced categorical metrics
                    results["categorical_metrics"][
                        col
                    ] = self._evaluate_categorical_diversity(col)
        ...
        try:
            ...
            results["uniqueness"] = {
                ...
                "relative_duplication": (
                    (duplicate_ratio / orig_duplicate_ratio) * 100
                    if orig_duplicate_ratio > 0
                    else 0
                ),
            }
    ...
    def _calculate_text_coverage(
        ...
        try:
            ...
            # Get vocabulary from synthetic data
            syn_tokens = []
            ...
            coverage = (len(common_vocab) / len(orig_vocab)) * 100
    ...
    def evaluate_text_diversity(self) -> Dict:
        ...
        for col in tqdm(self.text_columns, desc="Evaluating text columns"):
            try:
                # Calculate text coverage (vocabulary coverage)
                ...
                # Lexical diversity
                syn_lexical_metrics = self._evaluate_lexical_diversity(
                    self.synthetic_data, col
                )

                # Semantic diversity
                syn_semantic_metrics = self._evaluate_semantic_diversity(
                    self.synthetic_data, col
                )

                # Sentiment diversity
                syn_sentiment_metrics = self._evaluate_sentiment_diversity(
                    self.synthetic_data, col
                )
                ...
                cached_results = self.cache.load_cache(
                    "text_diversity", self.original_fingerprint, col
                )
                if cached_results is not None:
                    ...
                    # Lexical diversity
                    real_lexical_metrics = self._evaluate_lexical_diversity(
                        self.original_data, col
                    )

                    # Semantic diversity
                    real_semantic_metrics = self._evaluate_semantic_diversity(
                        self.original_data, col
                    )

                    # Sentiment diversity
                    real_sentiment_metrics = self._evaluate_sentiment_diversity(
                        self.original_data, col
                    )
                    ...
                    self.cache.save_cache(
                        "text_diversity", self.original_fingerprint, real_results, col
                    )

            except Exception as e:
                logger.error(
                    f"Error evaluating text diversity for column {col}: {str(e)}"
                )
                results["coverage"][col] = 0.0
                results["synthetic"][col] = {
                    "lexical_diversity": {},
    ...
    def _evaluate_lexical_diversity(self, data: pd.DataFrame, text_column: str) -> Dict:
        ...
        try:
            # Check cache first
            cached_result = self.cache.load_cache(
                "lexical_diversity", self.original_fingerprint, text_column
            )
            ...
            if len(df) == 0:
                ...
                result = {"sample_size": 0, "error": "No valid text data"}
                self.cache.save_cache(
                    "lexical_diversity", self.original_fingerprint, result, text_column
                )
                return result

            logger.info(
                f"Processing {len(df)} samples for lexical diversity (no sampling)"
            )
            ...
            # Cache the results
            self.cache.save_cache(
                "lexical_diversity", self.original_fingerprint, results, text_column
            )
    ...
    def _evaluate_semantic_diversity(
        self, data: pd.DataFrame, text_column: str
    ) -> Dict:
        ...
        try:
            ...
            cached_result = self.cache.load_cache(
                "semantic_diversity", self.original_fingerprint, text_column
            )
            if cached_result is not None:
                logger.info(
                    f"Using cached semantic diversity results for {text_column}"
                )
                return cached_result
            ...
            if n < 2:
                ...
                self.cache.save_cache(
                    "semantic_diversity", self.original_fingerprint, result, text_column
                )
                return result
            ...
            if self.device == "cuda" and TORCH_AVAILABLE:
                ...
                for i in range(n_batches):
                    for j in range(n_batches):
                        ...
                        if i < len(distances) and j < len(distances[i]):
                            dist_matrix[start_i:end_i, start_j:end_j] = distances[i][
                                :, : end_j - start_j
                            ]
                ...
                # CPU processing with progress updates
                ...
                for i in tqdm(
                    range(n_batches),
                    desc="Calculating distances (CPU)",
                    unit="batch",
                    dynamic_ncols=True,
                    leave=False,
                ):
                    ...
                    for j in range(i, n_batches):
                        ...
                        batch_dist = cosine_distances(batch_i, batch_j)
                        dist_matrix[start_i:end_i, start_j:end_j] = batch_dist
                        if i != j:
                            dist_matrix[start_j:end_j, start_i:end_i] = batch_dist.T
    ...
    def _evaluate_sentiment_diversity(
        self, data: pd.DataFrame, text_column: str
    ) -> Dict:
        ...
        try:
            # Check cache first
            cached_result = self.cache.load_cache(
                "sentiment_diversity", self.original_fingerprint, text_column
            )
            if cached_result is not None:
                logger.info(
                    f"Using cached sentiment diversity results for {text_column}"
                )
                return cached_result
            ...
            df = data.copy()
            df = df[df[text_column].str.strip().astype(bool)].dropna(
                subset=[text_column]
            )
            ...
            logger.info(
                f"Processing {len(df)} samples for sentiment diversity (no sampling)"
            )
            ...
            if "rating" not in df.columns:
                logger.warning(
                    "Rating column not found, using default sentiment analysis"
                )
                ...
                clf = clf.to(
                    device
                )  # Move to device (compatible with all Flair versions)
            ...
            try:
                counts = (
                    df.groupby("rating")["Sentiment Category"]
    ...
    def evaluate(self) -> Dict:
        ...
        if "text_diversity" in self.selected_metrics:
            ...
            if text_columns:
                ...
                logger.info(
                    "No text columns found in metadata, skipping text diversity evaluation"
                )
