...
def get_flair_model(model_name: str = 'flair/ner-english-large') -> SequenceTagger:
    """
    Get or load Flair model with caching and CPU-only device management.
    """
    with _model_lock:
...
class PrivacyEvaluator:
    ...
    def _process_entities_batch(self, texts: List[str]) -> List[Tuple]:
        """
        Process a batch of texts to extract named entities using Flair with CPU-only processing.
        """
        results = []
        ...
        with tqdm(total=len(texts), desc="Processing entities", unit="text") as pbar:
            for i in range(0, len(texts), batch_size):
                ...
                # Ensure model is on the correct device (no need to manually set token embeddings)
