...
class SynEvalDataFilter:
    def __init__(self, data: pd.DataFrame, metadata: Dict):
        """
        Data filter based on SynEval evaluation framework

        Args:
            data: Dataset to filter
            metadata: Metadata information of the dataset
        """
        self.data = data.copy()
        self.metadata = metadata
        self.logger = logging.getLogger(__name__)

        # Remove evaluator initialization as we don't need actual evaluators
    ...
    def calculate_text_length_scores(self, text_columns: List[str]) -> pd.Series:
        """Calculate text length scores (fidelity dimension)"""
        scores = pd.Series(0.0, index=self.data.index)

        if not text_columns:
            self.logger.warning(
                "No text columns specified, text length scores will be 0"
            )
            return scores
        ...
        for col in text_columns:
            if col in self.data.columns:
                ...
                self.logger.debug(
                    f"Text length range for column {col}: {text_lengths.min()} - {text_lengths.max()}"
                )

                # Normalize to 0-1 range
                if text_lengths.max() > text_lengths.min():
                    normalized_lengths = (text_lengths - text_lengths.min()) / (
                        text_lengths.max() - text_lengths.min()
                    )
                else:
                    normalized_lengths = pd.Series(0.5, index=text_lengths.index)
                scores += normalized_lengths
            else:
                self.logger.warning(f"Text column {col} does not exist in the data")

        return scores / len(text_columns) if text_columns else scores

    def calculate_sentiment_rating_alignment_scores(
        self, text_columns: List[str], rating_column: str
    ) -> pd.Series:
        """Calculate sentiment-rating alignment scores (utility dimension)"""
        scores = pd.Series(0.0, index=self.data.index)

        if rating_column not in self.data.columns:
            self.logger.warning(
                f"Rating column {rating_column} does not exist, skipping sentiment-rating alignment calculation"
            )
            return scores

        if not text_columns:
            self.logger.warning(
                "No text columns specified, sentiment-rating alignment scores will be 0"
            )
            return scores
        ...
        try:
            for col in text_columns:
                if col in self.data.columns:
                    ...
                    ratings = pd.to_numeric(
                        self.data[rating_column], errors="coerce"
                    ).fillna(3.0)

                    # Calculate sentiment polarity
                    sentiment_scores = texts.apply(
                        lambda x: TextBlob(x).sentiment.polarity
                    )

                    # Convert sentiment polarity to expected rating range
                    # Negative sentiment (-1 to 0) -> expected rating 1-3
                    # Neutral sentiment (0) -> expected rating 3
                    # Positive sentiment (0 to 1) -> expected rating 3-5
                    expected_ratings = 3.0 + sentiment_scores * 2.0

                    # Calculate alignment between actual and expected ratings
                    # Use negative absolute difference, smaller difference means higher score
                    alignment_scores = (
                        1.0 - abs(ratings - expected_ratings) / 4.0
                    )  # 4.0 is the maximum possible difference
                    alignment_scores = alignment_scores.clip(
                        0.0, 1.0
                    )  # Limit to 0-1 range

                    self.logger.debug(
                        f"Sentiment-rating alignment range for column {col}: {alignment_scores.min():.3f} - {alignment_scores.max():.3f}"
                    )
            ...
            self.logger.error(f"Error calculating sentiment-rating alignment: {e}")
            return pd.Series(0.5, index=self.data.index)

    def calculate_categorical_distribution_similarity(
        self, categorical_columns: List[str]
    ) -> pd.Series:
        """Calculate categorical distribution similarity scores (diversity dimension)"""
        scores = pd.Series(0.0, index=self.data.index)

        if not categorical_columns:
            self.logger.warning(
                "No categorical columns specified, categorical distribution similarity scores will be 0"
            )
            return scores
        ...
        for col in categorical_columns:
            if col in self.data.columns:
                ...
                # Calculate distribution similarity (simplified as rarity here, rarer categories score higher)
                # In practice, this should calculate similarity to ideal distribution
                rarity_scores = self.data[col].map(
                    lambda x: 1 - (value_counts.get(x, 0) / total_count)
                )
                scores += rarity_scores.fillna(0)

                self.logger.debug(
                    f"Categorical distribution similarity range for column {col}: {rarity_scores.min():.3f} - {rarity_scores.max():.3f}"
                )
            else:
                self.logger.warning(
                    f"Categorical column {col} does not exist in the data"
                )
    ...
    def calculate_semantic_diversity_scores(self, text_columns: List[str]) -> pd.Series:
        """Calculate semantic diversity scores (diversity dimension)"""
        scores = pd.Series(0.0, index=self.data.index)

        if not text_columns:
            self.logger.warning(
                "No text columns specified, semantic diversity scores will be 0"
            )
            return scores
        ...
        for col in text_columns:
            if col in self.data.columns:
                ...
                # Simplified semantic diversity calculation
                # 1. Vocabulary richness
                unique_words = texts.str.split().apply(
                ...
                sentence_count = sentences.apply(
                    lambda x: len([s for s in x if s.strip()]) if x else 1
                )
                avg_sentence_length = texts.str.len() / sentence_count

                # 3. Comprehensive semantic diversity score
                semantic_scores = (
                    vocabulary_richness
                    + avg_sentence_length / avg_sentence_length.max()
                ) / 2
                scores += semantic_scores.fillna(0.5)

                self.logger.debug(
                    f"Semantic diversity range for column {col}: {semantic_scores.min():.3f} - {semantic_scores.max():.3f}"
                )
            else:
                self.logger.warning(f"Text column {col} does not exist in the data")
    ...
    def calculate_named_entities_scores(self, text_columns: List[str]) -> pd.Series:
        """Calculate named entity scores (privacy dimension, fewer entities means higher score)"""
        scores = pd.Series(0.0, index=self.data.index)

        if not text_columns:
            self.logger.warning(
                "No text columns specified, named entity scores will be 0"
            )
            return scores
        ...
        try:
            ...
            for col in text_columns:
                if col in self.data.columns:
                    ...
                    # Calculate entity count for each text
                    entity_counts = []
                    for text in texts:
                    ...
                    # Convert to scores (fewer entities means higher score)
                    entity_counts = pd.Series(entity_counts)
                    if entity_counts.max() > 0:
                        # Normalize: fewer entities means higher score
                    ...
                    self.logger.debug(
                        f"Named entity score range for column {col}: {entity_scores.min():.3f} - {entity_scores.max():.3f}"
                    )
                else:
                    self.logger.warning(f"Text column {col} does not exist in the data")
            ...
            self.logger.warning(
                f"NER model unavailable, using simplified calculation: {e}"
            )
            # Simplified version: estimate based on text length and special character count
            for col in text_columns:
                if col in self.data.columns:
                    texts = self.data[col].astype(str)

                    # Estimate entity count based on text features
                    # Texts containing numbers, uppercase letters, special symbols may contain more entities
                    has_numbers = texts.str.contains(r"\d")
                    has_uppercase = texts.str.contains(r"[A-Z]")
                    has_special_chars = texts.str.contains(r"[^\w\s]")

                    # Comprehensive feature score (fewer features may mean fewer entities)
                    feature_scores = (
                        1.0
                        - (
                            has_numbers.astype(int)
                            + has_uppercase.astype(int)
                            + has_special_chars.astype(int)
                        )
                        / 3.0
                    )
                    ...
                    self.logger.debug(
                        f"Simplified named entity score range for column {col}: {feature_scores.min():.3f} - {feature_scores.max():.3f}"
                    )
                else:
                    self.logger.warning(f"Text column {col} does not exist in the data")
    ...
    def get_column_types(self) -> Dict[str, List[str]]:
        """获取按类型分组的列名"""
        column_types = {"text": [], "numerical": [], "categorical": [], "pii": []}
    ...
    def get_metadata_config(self) -> Dict:
        ...
        if "utility" in self.metadata:
            ...
            if "input_columns" in utility_config and "output_columns" in utility_config:
                ...
                self.logger.info(
                    f"从metadata获取utility配置: 输入={config['utility_input']}, 输出={config['utility_output']}"
                )
    ...
    def calculate_syn_eval_scores(self, config: Dict) -> pd.DataFrame:
        ...
        text_cols = (
            config.get("text_columns")
            or metadata_config.get("text_columns")
            or column_types["text"]
        )
        ...
        self.logger.info(
            f"文本长度得分范围: {text_length_scores.min():.3f} - {text_length_scores.max():.3f}"
        )

        # 1.2 Diagnostic overall score (fidelity的1/3权重)
        text_quality_scores = self.calculate_text_quality_proxy(text_cols)
        self.logger.info(
            f"诊断得分范围: {text_quality_scores.min():.3f} - {text_quality_scores.max():.3f}"
        )

        # 1.3 Quality overall score (fidelity的1/3权重)
        complexity_scores = self.calculate_text_complexity_proxy(text_cols)
        self.logger.info(
            f"质量得分范围: {complexity_scores.min():.3f} - {complexity_scores.max():.3f}"
        )

        # 计算Fidelity总分 (三个子指标的平均值)
        fidelity_scores = (
            text_length_scores + text_quality_scores + complexity_scores
        ) / 3
        scores_df["fidelity_score"] = fidelity_scores
        self.logger.info(
            f"Fidelity总分范围: {fidelity_scores.min():.3f} - {fidelity_scores.max():.3f}"
        )
        ...
        if utility_input_cols and utility_output_cols:
            ...
            sentiment_alignment_scores = (
                self.calculate_sentiment_rating_alignment_scores(
                    [input_col], output_col
                )
            )
        else:
            sentiment_alignment_scores = (
                self.calculate_sentiment_rating_alignment_scores(text_cols, rating_col)
            )

        scores_df["utility_score"] = sentiment_alignment_scores
        self.logger.info(
            f"Utility得分范围: {sentiment_alignment_scores.min():.3f} - {sentiment_alignment_scores.max():.3f}"
        )
        ...
        self.logger.info(
            f"分类分布相似性范围: {cat_dist_scores.min():.3f} - {cat_dist_scores.max():.3f}"
        )

        # 3.2 语义多样性 (diversity的1/2权重)
        semantic_scores = self.calculate_semantic_diversity_scores(text_cols)
        self.logger.info(
            f"语义多样性范围: {semantic_scores.min():.3f} - {semantic_scores.max():.3f}"
        )
        ...
        self.logger.info(
            f"Diversity总分范围: {diversity_scores.min():.3f} - {diversity_scores.max():.3f}"
        )
        ...
        self.logger.info(
            f"Privacy得分范围: {named_entities_scores.min():.3f} - {named_entities_scores.max():.3f}"
        )
    ...
    def calculate_text_quality_proxy(self, text_columns: List[str]) -> pd.Series:
        ...
        for col in text_columns:
            if col in self.data.columns:
                ...
                punctuation_scores = 1 - abs(
                    punctuation_count - punctuation_count.median()
                ) / (punctuation_count.max() + 1)

                col_scores = (
                    length_scores + vocabulary_scores + punctuation_scores
                ) / 3
    ...
    def calculate_text_complexity_proxy(self, text_columns: List[str]) -> pd.Series:
        ...
        for col in text_columns:
            if col in self.data.columns:
                ...
                long_words = words.apply(
                    lambda x: sum(1 for w in x if len(w) > 6) if x else 0
                )
...
def main():
    ...
    filtered_data, filtered_scores = data_filter.filter_top_data(
